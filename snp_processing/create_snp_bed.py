...
def format_gwas_df(gwas_df):
    ...
    print(f'{gwas_df["CHR_POS"].isnull().mean() * 100:.3f}% of rows have null values for "CHR_POS" and "CHR_ID".')
    ...
    gwas_df['CHR_POS'] = gwas_df['CHR_POS'].apply(lambda s: re.split(r'[ ;x]', str(s).replace(' ', '')))
    gwas_df['CHR_ID'] = gwas_df['CHR_ID'].apply(lambda s: re.split(r'[ ;x]', str(s).replace(' ', '')))
...
def create_snp_bed_from_gwas_df(formatted_gwas_df, snp_region_side_length):
    ...
    for (trait, chrom), group_df in tqdm.tqdm(trait_chrom_group, total=trait_chrom_group.ngroups,
        ...
        for idx, row in group_df_sorted.iterrows():
            ...
            if any([((str(chrom) == str(k)) and (snp_loc in v)) for k, v in exclude_ranges.items()]):
            ...
            start_pos, end_pos = create_snp_region(snp_loc=snp_loc, snp_region_side_length=snp_region_side_length,
                                                   chrom=f'chr{chrom}')
            ...
            trait_linkage_data.append({
                'trait': trait,
                'snp': snp_name
            })

    snp_bed_df = pd.DataFrame(snp_data)[['chrom', 'chromStart', 'chromEnd', 'name']].drop_duplicates()
    ...
    print(f'Dropped SNPs {len(dropped_mhc):,} times ({len(set(dropped_mhc)):,} unique SNPs) from a trait because they '
          f'occurred in the MHC.')
    print(f'Dropped SNPs {len(dropped_overlap):,} times ({len(set(dropped_overlap)):,} unique SNPs) from a trait '
    ...
    n_snps_before_filtering = ('snp_chr' + formatted_gwas_df['CHR_ID'] + '_' +
                               formatted_gwas_df['CHR_POS'].astype(str)).nunique()
    n_snps_after_filtering = snp_bed_df['name'].nunique()
    print(f'The number of unique SNPs went from {n_snps_before_filtering:,} before filtering to '
    ...
    print(f'The number of unique traits went from {n_traits_before_filtering:,} before filtering to '
...
def create_fake_snp_bed(snp_bed, snp_window_side_length, existing_fake_snp_bed=None, offset=100_000, seed=1234567):
    ...
    for idx, row in tqdm.tqdm(bed_cp.iterrows(), desc='Creating fake SNPs', total=bed_cp.shape[0]):
        ...
        if (existing_fake_snp_bed is not None) and ((snp_name + '_fake') in existing_fake_snp_bed.index):
            ...
            if snp_loc <= offset:  # Subtracting the offset would result in a negative position for the SNP
                options = [offset]
        ...
        fake_snp_start_pos, fake_snp_end_pos = create_snp_region(snp_loc=fake_snp_loc,
                                                                 snp_region_side_length=snp_window_side_length,
                                                                 chrom=snp_chrom)
        fake_snp_data.append({'chrom': snp_chrom, 'chromStart': fake_snp_start_pos, 'chromEnd': fake_snp_end_pos,
                              'name': snp_name + '_fake', 'fake_snp_loc': fake_snp_loc})
...
def create_snp_beds(gwas_df, snp_region_side_lengths):
    ...
    for idx, side_length in enumerate(sorted_snp_region_side_lengths):
        snp_bed, trait_linkage_df = create_snp_bed_from_gwas_df(formatted_gwas_df=gwas_df_formatted.copy(deep=True),
                                                                snp_region_side_length=side_length)
        if idx == 0:
            existing_fake_snp_bed = None
        else:
            # Get the locations of fake SNPs that have already been generated.
            existing_fake_snp_bed = pd.concat([df.drop(columns=['chromStart', 'chromEnd'])
                                               for df in fake_beds.values()]).drop_duplicates()
            assert not existing_fake_snp_bed['name'].duplicated().any()
        # The function create_fake_snp_bed reuses previously-generated fake SNPs when possible.
        fake_snp_bed = create_fake_snp_bed(
            snp_bed=snp_bed, snp_window_side_length=side_length,
    ...
    for (ix1, ix2) in itertools.product(range(len(fake_beds)), range(len(fake_beds))):
        ...
        assert (bed_1.set_index('name').loc[list(common_snps)]['fake_snp_loc'] ==
                bed_2.set_index('name').loc[list(common_snps)]['fake_snp_loc']).all()

    return real_beds, fake_beds, trait_linkage_dfs


def main(gwas_snp_df_fpath, out_dir):
    path_helper = PathHelper(out_dir=out_dir)
    gwas_snp_df = pd.read_csv(gwas_snp_df_fpath, sep='\t')

    real_beds, fake_beds, trait_linkage_dfs = create_snp_beds(gwas_df=gwas_snp_df,
                                                              snp_region_side_lengths=const.SNP_REGION_SIDE_LENGTHS)
    ...
    for window_size in real_beds.keys():
        ...
        window_real_bed['chr_proc'] = window_real_bed['chrom'].apply(
            lambda c: f'{int(c.lstrip("chr")):02}' if c.lstrip("chr").isnumeric() else c.lstrip("chr"))
        window_real_bed = window_real_bed.sort_values(by=['chr_proc', 'chromStart'], ascending=True).drop(
            columns=['chr_proc'])

        window_fake_bed = fake_beds[window_size][['chrom', 'chromStart', 'chromEnd', 'name']]
        window_fake_bed['chr_proc'] = window_fake_bed['chrom'].apply(
            lambda c: f'{int(c.lstrip("chr")):02}' if c.lstrip("chr").isnumeric() else c.lstrip("chr"))
        window_fake_bed = window_fake_bed.sort_values(by=['chr_proc', 'chromStart'], ascending=True).drop(
            columns=['chr_proc'])

        assert set(window_real_bed['name'].values) == \
               set(window_fake_bed['name'].apply(lambda s: s.replace('_fake', '')).values)

        window_real_bed.to_csv(path_helper.get_snp_bed_fpath(window_size=window_size, fake=False), index=False,
                               header=None, sep='\t')
        window_fake_bed.to_csv(path_helper.get_snp_bed_fpath(window_size=window_size, fake=True), index=False,
                               header=None, sep='\t')
        trait_linkage_dfs[window_size].to_csv(path_helper.get_trait_snp_linkage_df_fpath(window_size=window_size))
