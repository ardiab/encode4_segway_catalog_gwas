...
def calculate_sample_metrics(sample_id, data_loader, snp_region_side_length, fake_snps):
    ...
    mean_caas_series = calculate_mean_caas(label_distr_df=sample_label_distr_df,
                                           label_caas_dict=sample_label_caas_dict, sample_id=sample_id)
...
def create_metric_dfs(metric_series_dict):
    ...
    assert all([set(metric_names) == set(metric_series_dict[sample_id]) for sample_id in sample_ids]), \
        f'Not all samples have results for the same metrics.'

    for metric_name in metric_names:
        metric_df = pd.concat([metric_series_dict[sample_id][metric_name] for sample_id in sample_ids], axis=1)
...
def main(out_dir):
    ...
    assert not os.path.isdir(base_enrichment_dir), f'Directory {base_enrichment_dir} already exists.'
    ...
    for snp_region_side_length, fake_snps in itertools.product(const.SNP_REGION_SIDE_LENGTHS, [True, False]):
        ...
        for sample_id in meta_helper.iter_samples(show_progress=True):
            sample_metrics[sample_id] = calculate_sample_metrics(sample_id=sample_id, data_loader=data_loader,
        ...
        for metric_name, metric_df in metric_df_dict.items():
            metric_df_fpath = path_helper.get_snp_metric_df_fpath(metric_name=metric_name,
                                                                  window_size=snp_region_side_length, fake=fake_snps)
