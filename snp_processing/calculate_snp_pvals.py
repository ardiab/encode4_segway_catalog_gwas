...
def _get_linkage_idxs(snp_metric_df, trait_snp_linkage_df, fake):
    ...
    for trait, trait_linkage_df in trait_snp_linkage_df.groupby('trait'):
        trait_snp_names = [snp + '_fake' if fake else snp for snp in trait_linkage_df['snp'].unique()]
...
def _calculate_signedrank_pvals_uniform_null(snp_metric_df, trait_snp_linkage_df, fake):
    data = []
    linkage_idxs = _get_linkage_idxs(snp_metric_df=snp_metric_df, trait_snp_linkage_df=trait_snp_linkage_df, fake=fake)
    ...
    for trait_name, trait_snp_idxs in tqdm(linkage_idxs.items()):
        for sample_idx, sample_id in enumerate(snp_metric_df.columns):
            ...
            data.append({'trait': trait_name, 'test_stat': test_stat, 'exp_median': exp_median, 'pval': pval,
                         'sample': sample_id})
...
def _calculate_signedrank_pvals_sample_specific_null(snp_metric_df, trait_snp_linkage_df, fake):
    ...
    for trait_name, trait_snp_idxs in tqdm(linkage_idxs.items()):
        for sample_idx, sample_id in enumerate(snp_metric_df.columns):
            trait_sample_ranks = rank_array[trait_snp_idxs, sample_idx]
            exp_median = np.median(trait_sample_ranks)
            test_stat, pval = wilcoxon(trait_sample_ranks - sample_medians[sample_id], alternative='greater')
            data.append({'trait': trait_name, 'test_stat': test_stat, 'exp_median': exp_median, 'pval': pval,
                         'sample': sample_id})
...
def calculate_signedrank_pvals(snp_metric_df, trait_snp_linkage_df, fake):
    ...
    test_stats_uniform_null, pvals_uniform_null, medians_uniform_null = _calculate_signedrank_pvals_uniform_null(
...
def calculate_sample_pvals(metric_name, snp_region_side_length, fake_snps, data_loader, trait_snp_linkage_df):
    metric_df = data_loader.load_snp_metric_df(metric_name=metric_name, snp_region_side_length=snp_region_side_length,
                                               fake=fake_snps)
    metric_df_snp_names = metric_df.index.tolist() if not fake_snps else \
        [i.replace('_fake', '') for i in metric_df.index.tolist()]
    ...
    test_result_dict = calculate_signedrank_pvals(snp_metric_df=metric_df_sub,
...
def get_filtered_trait_snps(snp_region_side_length, snp_cutoff, data_loader):
    ...
    sorted_traits_fwd = trait_snp_count_df[trait_snp_count_df['trait'].isin(traits_to_keep)].sort_values(
        by=['n_snps'], ascending=False)['trait'].tolist()
    sorted_traits_rev = trait_snp_count_df[trait_snp_count_df['trait'].isin(traits_to_keep)].sort_values(
        by=['n_snps'], ascending=True)['trait'].tolist()
    results = {}
    for direction_traits, direction_desc in [(sorted_traits_fwd, 'fwd'), (sorted_traits_rev, 'rev')]:
        added_traits = []
        added_snps = set()
        overlap_data = []
...
def main(out_dir, snp_cutoff):
    path_helper = PathHelper(out_dir=out_dir)
    data_loader = DataLoader(out_dir=out_dir)

    for metric_name in ['mean_caas']:
        for snp_region_side_length, fake_snps in itertools.product(const.SNP_REGION_SIDE_LENGTHS, [True, False]):
            trait_snp_linkage_df = get_filtered_trait_snps(snp_region_side_length=snp_region_side_length,
                                                           snp_cutoff=snp_cutoff, data_loader=data_loader)

            metric_test_result_dict = calculate_sample_pvals(metric_name=metric_name,
                                                             snp_region_side_length=snp_region_side_length,
                                                             fake_snps=fake_snps,
                                                             data_loader=data_loader,
                                                             trait_snp_linkage_df=trait_snp_linkage_df)
            for test_type, test_df_dict in metric_test_result_dict.items():
                pval_df_fpath = path_helper.get_pval_df_fpath(window_size=snp_region_side_length,
                                                              metric_name=metric_name,
                                                              test_type=test_type,
                                                              fake=fake_snps)
                test_df_dict['pval'].to_pickle(pval_df_fpath)
                test_stat_df_fpath = path_helper.get_test_stat_df_fpath(window_size=snp_region_side_length,
                                                                        metric_name=metric_name,
                                                                        test_type=test_type,
                                                                        fake=fake_snps)
                test_df_dict['test_stat'].to_pickle(test_stat_df_fpath)
                effect_size_df_fpath = path_helper.get_effect_size_df_fpath(window_size=snp_region_side_length,
                                                                            metric_name=metric_name,
                                                                            test_type=test_type,
                                                                            fake=fake_snps)
