# This script should not be directly called by the user. Instead, use compute_caas.py, which calls this script.
import argparse
import json
import os
...
def compute_caas(sample_id, chrom_cons_array_dict, data_loader, path_helper):
    ann_bed_df = data_loader.load_biosample_annotation_bed(sample_id=sample_id, drop_alternate_loci=True)
    ...
    for label, label_df in ann_bed_df.groupby('label'):
        ...
        for chrom, chrom_df in label_df.groupby('chrom'):
            ...
            chrom_idxs = [(row['chrom_start'], row['chrom_end']) for idx, row in chrom_df.iterrows()]
    ...
    for label, cons_arr in label_caas.items():
        ...
        label_color = const.LABEL_COLOR_MAP[interp_term] if interp_term.lower() != 'quiescent' else (0.0, 0.0, 0.0)
        counts, bins, bars = ax.hist(np.clip(abs_concat_array, a_min=None, a_max=label_percentiles[label][99]),
                                     density=True, cumulative=True, label=label, histtype='step', bins=30,
                                     color=label_color)
        label_plot_info[label] = {'counts': list(counts), 'bins': list(bins)}

    ax.axhline(0.75, color='black', linestyle='--', alpha=0.4)
    ax.set_yticks(np.arange(0, 1.05, 0.05))
    ax.set_xlabel('Absolute PhyloP score\n(clipped to remove values >= 99th percentile)', fontsize=14, weight='bold')
    ax.set_ylabel('eCDF', fontsize=14, weight='bold')
    ax.set_title(f'Label PhyloP eCDFs', fontsize=18, weight='bold')
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    fig.savefig(os.path.join(sample_out_dir, f'label_phylop_distr_hist.png'))
    plt.close(fig)
    ...
    coverage_normed = (ann_bed_df.groupby('label')['length'].sum() / ann_bed_df['length'].sum()).to_dict()
...
def main(out_dir, sample_id_list_fpath):
    ...
    for chrom in const.CHROM_LENGTHS.keys():
        ...
        chrom_cons_array_dict[chrom] = data_loader.load_phylop_array(array_fpath=chrom_phylop_array_fpath, chrom=chrom)

    for sample_id in tqdm.tqdm(sample_ids, desc='Computing sample CAAS'):
        compute_caas(sample_id=sample_id, chrom_cons_array_dict=chrom_cons_array_dict, data_loader=data_loader,
                     path_helper=path_helper)


if __name__ == '__main__':
    arg_dict = parse_args()
    main(out_dir=arg_dict['out_dir'], sample_id_list_fpath=arg_dict['sample_id_list_fpath'])
