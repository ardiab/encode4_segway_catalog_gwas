...
def get_snp_window_contents(snp_anns, window_size, label_idx_dict):
    ...
    pos = 0
    for _, row in snp_anns.iterrows():
...
def create_snp_window_heatmap(snp_contents, title, label_idx_dict, ax):
    ...
    ax.legend(
        handles=[
            matplotlib.patches.Patch(color=const.LABEL_COLOR_MAP[label], label=label)
            for label in const.LABEL_COLOR_MAP.keys()
        ],
        loc="center left",
...
if __name__ == "__main__":
    ...
    mean_caas_df = data_loader.load_snp_metric_df(
        metric_name="mean_caas", snp_region_side_length=WINDOW_SIZE, fake=False
    )

    # These beds are for sample ENCSR011EQM
    real_bed = data_loader.load_intersection_bed(
        os.path.join(arg_dict["out_dir"], "ENCSR111ABE_real_snp_ann_intersections_10000.bed.gz")
    )
    real_bed = real_bed.sort_values(by=["snp_name", "ann_start"], ascending=True).set_index(
        "snp_name"
    )
    ...
    for idx, snp_name in tqdm(enumerate(trait_snps), total=len(trait_snps)):
        ...
        snp_window_contents = get_snp_window_contents(
            snp_anns=real_bed.loc[snp_name], window_size=WINDOW_SIZE, label_idx_dict=label_idxs
        )
        assert all(snp_window_contents.sum(axis=0) > 0)
        title = f"{snp_name} (Mean CAAS = {mean_caas_df.loc[snp_name, accession]:.3f})"
        create_snp_window_heatmap(
            snp_window_contents, title=title, label_idx_dict=label_idxs, ax=ax
        )

    fig.tight_layout()
    fig.suptitle(f"Cell Type: {ctype}\nTrait: {trait} ({len(trait_snps):,} SNPs)")
    fig.savefig(os.path.join(arg_dict["out_dir"], "snp_region_labels.pdf"), format="pdf")
