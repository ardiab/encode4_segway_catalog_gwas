            ...
            data.append(
                {
                    ...
                    "overlap": len(intersection) / len(trait_snps),
...
def get_neg_log_pval_heatmap(
    pval_df,
    fig_title,
    meta_helper,
    sig_threshold,
    trait_subset_to_display=None,
    ...
    clustermap = sns.clustermap(pval_df_proc.values, dendrogram_ratio=[0.05, 0.05], cmap="magma")
    ...
    n_traits_to_display = (
        len(trait_subset_to_display) if trait_subset_to_display is not None else pval_df.shape[0]
    )
    ...
    if trait_subset_to_display is not None:
        ...
        clustered_df_sub = clustered_df_sub[
            clustered_df_sub["trait"].isin(trait_subset_to_display)
        ].set_index("trait")
    else:
        clustered_df_sub = clustered_df

    sns.heatmap(
        clustered_df_sub.values,
        cmap=cmap,
        ax=ax,
        vmin=proc_sig_threshold,
        linewidths=0.0,
        rasterized=True,
        square=True,
        cbar=False,
    ...
    xtick_labels = [
        meta_helper.get_sample_description(sample_id)
        for sample_id in list(clustered_df_sub.columns)
    ]
...
def draw_mean_caas_boxplot(mean_caas_df, cell_name, trait_name, trait_snp_linkage_df):
    ...
    other_snps = trait_snp_linkage_df[~(trait_snp_linkage_df["snp"].isin(trait_snps))][
        "snp"
    ].unique()
    ...
    sns.boxplot(
        x=["Trait SNPs"] * len(snp_caas) + ["Other SNPs"] * len(other_caas),
        y=snp_caas + other_caas,
        ax=ax,
    )
...
def get_filtered_trait_snps(snp_region_side_length, snp_cutoff, data_loader):
    ...
    for direction_traits, direction_desc in [
        (sorted_traits_desc, "descending"),
        (sorted_traits_asc, "ascending"),
    ]:
        added_traits = []
        ...
        for trait in tqdm(direction_traits):
            ...
            if len(added_snps_pairwise) > 0:
                overlap_pairwise = max(
                    [len(trait_snps.intersection(s)) / len(trait_snps) for s in added_snps_pairwise]
                )
            else:
                overlap_pairwise = 0
            overlap_data.append(
                {
                    ...
                    "overlap_pairwise": overlap_pairwise,
                }
            )
    ...
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.hist(
        results["descending"]["overlap_df"]["overlap_union"].values,
        label="Descending -- Union",
        alpha=0.6,
    )
    ax.hist(
        results["ascending"]["overlap_df"]["overlap_union"].values,
        label="Ascending -- Union",
        alpha=0.6,
    )
    ax.hist(
        results["descending"]["overlap_df"]["overlap_pairwise"].values,
        label="Descending -- Pairwise",
        alpha=0.6,
    )
    ax.hist(
        results["ascending"]["overlap_df"]["overlap_pairwise"].values,
        label="Ascending -- Pairwise",
        alpha=0.6,
    )
...
if __name__ == "__main__":
    ...
    trait_snp_count_dict = (
        data_loader.load_trait_snp_count_df(window_size=window_size)
        .set_index("trait")["n_snps"]
        .to_dict()
    )
    trait_filter_dict = get_filtered_trait_snps(
        snp_region_side_length=10_000, snp_cutoff=30, data_loader=data_loader
    )
    ...
    for sort_direction, overlap_filtering_method in [("descending", "overlap_pairwise")]:
        ...
        traits_to_keep_after_overlap_filtering = overlap_df[
            overlap_df[overlap_filtering_method] < overlap_threshold
        ]["trait"].unique()
        traits_to_keep_after_sig_count_filtering = pval_df[
            (pval_df < sig_threshold).sum(axis=1) >= min_n_significant_req
        ].index.unique()

        traits_to_cluster_on = traits_to_keep_after_sig_count_filtering
        trait_subset_to_display = list(
            set(traits_to_cluster_on).intersection(set(traits_to_keep_after_overlap_filtering))
        )

        pval_df = pval_df.loc[traits_to_cluster_on]
        full_fig_title = (
            f"-log10 P-value (trait X biosample) heatmap\n"
        ...
        fig_full, clustered_traits_full = get_neg_log_pval_heatmap(
            pval_df=pval_df,
            fig_title=full_fig_title,
            meta_helper=meta_helper,
            sig_threshold=sig_threshold,
            trait_subset_to_display=None,
            trait_snp_count_dict=trait_snp_count_dict,
        )
        ...
        fig_full.savefig(
            os.path.join(arg_dict["out_dir"], full_fig_out_path.split("/")[-1]), format="pdf"
        )
        fig_full.savefig(
            os.path.join(arg_dict["out_dir"], full_fig_out_path.split("/")[-1]) + ".png"
        )
        plt.close(fig_full)
        partial_fig_title = (
            f"-log10 P-value (trait X biosample) heatmap\n"
        ...
        fig_partial, clustered_traits_partial = get_neg_log_pval_heatmap(
            pval_df=pval_df,
            fig_title=partial_fig_title,
            meta_helper=meta_helper,
            sig_threshold=sig_threshold,
            trait_subset_to_display=trait_subset_to_display,
            trait_snp_count_dict=trait_snp_count_dict,
        )
        ...
        fig_partial.savefig(
            os.path.join(arg_dict["out_dir"], partial_fig_out_path.split("/")[-1]), format="pdf"
        )
        fig_partial.savefig(
            os.path.join(arg_dict["out_dir"], partial_fig_out_path.split("/")[-1]) + ".png"
        )
