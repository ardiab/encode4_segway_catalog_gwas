...
def download_and_unzip_phylop_wigfix_files(out_dir):
    ...
    for wig in tqdm.tqdm(const.PHYLOP_WIGS, desc="Downloading PhyloP wigFix files"):
        subprocess.call(
            ["wget", "-q", "-P", out_dir, f"{const.PHYLOP_BASE_URL}/{wig}.gz"],
            stderr=subprocess.STDOUT,
        )
        subprocess.call(
            ["gzip", "-d", f"{os.path.join(out_dir, wig)}.gz"],
            stderr=subprocess.STDOUT,
...
def wiggle_to_numpy(wiggle_fpath):
    ...
    for ix, wiggle_line_start_idx in enumerate([info_line[0] for info_line in info_lines]):
        if ix < len(info_lines) - 1:
            ...
            phylop_vals = [
                float(val) for val in wiggle_data[wiggle_line_start_idx + 1 : wiggle_line_end_idx]
            ]
        else:
        ...
        chrom_start_idx = (
            int(re.findall(r"start=(.+?)\b", wiggle_data[wiggle_line_start_idx])[0]) - 1
        )
...
def process_phylop(base_out_dir):
    ...
    try:
        ...
        phylop_wiggle_files = {
            chrom: path_helper.get_phylop_wiggle_fpath(chrom)
            for chrom in const.CHROM_LENGTHS.keys()
        }
        assert all([os.path.isfile(f) for f in phylop_wiggle_files.values()])
        for chrom, wiggle_path in tqdm.tqdm(
            phylop_wiggle_files.items(), desc="Converting wiggle files to numpy arrays"
        ):
            run_phylop_processing(
                phylop_wiggle_fpath=wiggle_path,
                phylop_numpy_fpath=path_helper.get_phylop_numpy_fpath(chrom=chrom),
                chrom=chrom,
            )
    except AssertionError:
        phylop_numpy_fpaths = {
            chrom: path_helper.get_phylop_numpy_fpath(chrom) for chrom in const.CHROM_LENGTHS.keys()
        }
