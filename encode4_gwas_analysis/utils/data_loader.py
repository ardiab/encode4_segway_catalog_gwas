...
from encode4_gwas_analysis.utils.constants import CHROM_LENGTHS
...
class DataLoader:
    ...
    def validate_annotation_bed(cls, df, allow_alternate_loci=False):
        if not allow_alternate_loci:
            ...
            assert (
                df[cls.BED_COLS[0]]
                .apply(lambda c: c.split("_")[0])
                .isin([f"chr{i}" for i in range(23)] + ["chrX", "chrY"])
                .all()
            )
    ...
    def validate_intersection_bed(cls, df):
        ...
        assert (
            df[["ann_start", "ann_end", "snp_start", "snp_end", "intersection_length"]].dtypes
            == np.int64
        ).all()
    ...
    def load_sample_intersection_bed(self, sample_id, window_size, fake):
        ...
        return pd.read_pickle(
            self.path_helper.get_sample_intersection_bed_fpath(
                sample_id=sample_id, window_size=window_size, fake=fake
            )
        )

    def load_sample_snp_label_distribution(self, sample_id, snp_region_side_length, fake):
        distr_df_fpath = self.path_helper.get_label_distr_df_fpath(
            sample_id=sample_id, window_size=snp_region_side_length, fake=fake
        )
    ...
    def load_sample_label_coverage(self, sample_id, normalized):
        label_coverage_fpath = self.path_helper.get_label_coverage_json_fpath(
            sample_id=sample_id, normalized=normalized
        )
        with open(label_coverage_fpath, "r") as in_f:
            return json.load(in_f)

    def load_sample_label_phylop_percentiles(self, sample_id):
        percentile_fpath = self.path_helper.get_label_phylop_percentiles_json_fpath(
            sample_id=sample_id
        )
    ...
    def load_snp_bed(self, snp_region_side_length, fake):
        snp_bed_fpath = self.path_helper.get_snp_bed_fpath(
            window_size=snp_region_side_length, fake=fake
        )
    ...
    def load_snp_metric_df(self, metric_name, snp_region_side_length, fake):
        metric_df_fpath = self.path_helper.get_snp_metric_df_fpath(
            metric_name=metric_name, window_size=snp_region_side_length, fake=fake
        )
        return pd.read_pickle(metric_df_fpath)

    def load_trait_snp_linkage_df(self, window_size):
        snp_linkage_df_fpath = self.path_helper.get_trait_snp_linkage_df_fpath(
    ...
    def load_trait_snp_count_df(self, window_size):
        ...
        assert (
            trait_snp_linkage_df.groupby("trait")
            .apply(lambda trait_df: trait_df["snp"].nunique() == trait_df.shape[0])
            .all()
        )

        return (
            trait_snp_linkage_df.groupby("trait")["snp"]
            .nunique()
            .reset_index()
    ...
    def load_pval_df(self, metric_name, test_type, window_size, fake):
        pval_df_fpath = self.path_helper.get_pval_df_fpath(
            metric_name=metric_name, test_type=test_type, window_size=window_size, fake=fake
        )

        return pd.read_pickle(pval_df_fpath)

    def load_test_stat_df(self, metric_name, test_type, window_size, fake):
        test_stat_df_fpath = self.path_helper.get_test_stat_df_fpath(
            metric_name=metric_name, test_type=test_type, window_size=window_size, fake=fake
        )
    ...
    def load_effect_size_df(self, metric_name, test_type, window_size, fake):
        effect_size_df_fpath = self.path_helper.get_effect_size_df_fpath(
            metric_name=metric_name, test_type=test_type, window_size=window_size, fake=fake
        )
