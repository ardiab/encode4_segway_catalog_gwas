...
def run_intersection(
    sample_ann_bed_fpath,
    real_snp_bed_fpath,
    fake_snp_bed_fpath,
    snp_region_side_length,
    sample_id,
    path_helper,
):
    ...
    os.system(
        f"bedtools intersect -a {sample_ann_bed_fpath} -b {real_snp_bed_fpath}  "
    ...
    os.system(
        f"bedtools intersect -a {sample_ann_bed_fpath} -b {fake_snp_bed_fpath}  "
        f"-wo > {out_fake_snp_intersection_bed_fpath}"
    )


def zip_intersection_beds(sample_id, snp_region_side_length, path_helper):
    real_snp_intersection_bed_fpath = path_helper.get_sample_intersection_bed_fpath(
        sample_id=sample_id, window_size=snp_region_side_length, zipped=False, fake=False
    )
    os.system(f"gzip {real_snp_intersection_bed_fpath}")

    fake_snp_intersection_bed_fpath = path_helper.get_sample_intersection_bed_fpath(
        sample_id=sample_id, window_size=snp_region_side_length, zipped=False, fake=True
    )
...
def get_snp_bed_label_distribution(bed_fpath):
    ...
    snp_label_counts = (
        intersection_bed.groupby(["snp_name", "ann_label"])["intersection_length"]
        .sum()
        .reset_index()
    )
    snp_label_counts = snp_label_counts.pivot(
        index="snp_name", columns="ann_label", values="intersection_length"
    ).fillna(0)
...
def get_snp_region_label_distributions(sample_id, snp_region_side_length, path_helper):
    ...
    real_snps_label_distr = get_snp_bed_label_distribution(
        bed_fpath=out_real_snp_intersection_bed_fpath
    )
    fake_snps_label_distr = get_snp_bed_label_distribution(
        bed_fpath=out_fake_snp_intersection_bed_fpath
    )

    real_snps_label_distr_fpath = path_helper.get_label_distr_df_fpath(
        sample_id=sample_id, window_size=snp_region_side_length, fake=False
    )
    fake_snps_label_distr_fpath = path_helper.get_label_distr_df_fpath(
        sample_id=sample_id, window_size=snp_region_side_length, fake=True
    )
...
def main(out_dir, sample_id_list_fpath):
    ...
    for sample_id in tqdm.tqdm(sample_ids, desc="Intersecting SNPs"):
        ...
        for snp_region_side_length in const.SNP_REGION_SIDE_LENGTHS:
            fake_snp_bed_fpath = path_helper.get_snp_bed_fpath(
                window_size=snp_region_side_length, fake=True
            )
            real_snp_bed_fpath = path_helper.get_snp_bed_fpath(
                window_size=snp_region_side_length, fake=False
            )
            assert os.path.isfile(real_snp_bed_fpath)
            assert os.path.isfile(fake_snp_bed_fpath)

            run_intersection(
                sample_ann_bed_fpath=sample_ann_bed_fpath,
                real_snp_bed_fpath=real_snp_bed_fpath,
                fake_snp_bed_fpath=fake_snp_bed_fpath,
                snp_region_side_length=snp_region_side_length,
                sample_id=sample_id,
                path_helper=path_helper,
            )
            get_snp_region_label_distributions(
                sample_id=sample_id,
                snp_region_side_length=snp_region_side_length,
                path_helper=path_helper,
            )
            zip_intersection_beds(
                sample_id=sample_id,
                snp_region_side_length=snp_region_side_length,
                path_helper=path_helper,
            )
