...
def plot_pval_ranks(sample_pval_series, trait_name, sig_threshold, ax, meta_helper, cap_pvals_at_1=True):
    rank_counter = 1
    ...
    if len(sig_traits) > 0:
        ax.scatter(range(rank_counter, len(sig_traits) + rank_counter), sig_traits.values, color='red')
        rank_counter += len(sig_traits)
    ax.scatter(range(rank_counter, len(insig_traits) + rank_counter), insig_traits.values, color='blue')
    ...
    ax.set_title(' '.join([w.capitalize() for w in trait_name.split(' ')]), fontsize=18, weight='bold')
    ax.tick_params(axis='both', which='major', labelsize=18)
    red_circle = plt.Line2D([0], [0], marker='o', color='white', markerfacecolor='red', markersize=10,
                            label='Significant association')
    blue_circle = plt.Line2D([0], [0], marker='o', color='white', markerfacecolor='blue', markersize=10,
    ...
    for i in range(top_n_ranks):
        sample_name = meta_helper.get_sample_description(trait_pvals.index.values[i]).split('Homo sapiens ')[-1]
    ...
    ax.annotate(annotation, color='black', xy=(150, 0.05),
                xytext=(233, 0.055), fontsize=12, horizontalalignment='right')
    ax.set_xlim([1, 235])
...
def main(out_dir):
    ...
    if all_traits:
        ...
        sorted_trait_list = trait_snp_count_df[trait_snp_count_df['n_snps'] >= 30].sort_values(
            by=['n_snps'], ascending=False)['trait'].unique().tolist()
        n_cols = 10
        n_rows = 10
        split_point = (len(sorted_trait_list) % (n_cols * n_rows))
        trait_groups = list(np.array_split(sorted_trait_list[:-split_point],
        ...
        for fig_idx, trait_group in tqdm(enumerate(trait_groups), total=len(trait_groups)):
            trait_min_n_snps = trait_snp_count_df[trait_snp_count_df['trait'].isin(trait_group)]['n_snps'].min()
            trait_max_n_snps = trait_snp_count_df[trait_snp_count_df['trait'].isin(trait_group)]['n_snps'].max()
            ...
            for idx, trait in enumerate(trait_group):
                ...
                ax.plot([rank_start + 0.6, rank_start + 0.6], [max_pval + 5, max_pval + 3], color='black')
                ...
                ax.annotate('', xy=((rank_start + rank_end) / 2, max_pval + 5),
                ...
                if len(insig_annotation) > 0:
                    ...
                    ax.annotate(insig_annotation, color='blue', xy=((rank_start + rank_end) / 2 + 2, max_pval),
                                xytext=(rank_start + 3, max_pval + 10))
                else:
                    n_insig = 0
                ax.annotate(sig_annotation, color='red', xy=((rank_start + rank_end) / 2 + 2, max_pval),
                            xytext=(rank_start + 3, max_pval + 10 + (7 * n_insig)))
                ax.set_xlim([1, 235])
                ax.set_ylim([0, 234])
                ax.legend(handles=[red_circle, blue_circle])

            fig.savefig(os.path.join(out_dir, f'{fig_idx}_{trait_min_n_snps}_{trait_max_n_snps}.pdf'), format='pdf')
            fig.savefig(os.path.join(out_dir, f'{fig_idx}_{trait_min_n_snps}_{trait_max_n_snps}.png'))
    else:
        ...
        for trait in trait_subset:
            ...
            trait_pval_df = trait_pvals.sort_values(ascending=True).reset_index().rename(columns={trait: 'pval'})
            trait_pval_df['sample'] = trait_pval_df['sample'].apply(lambda s: meta_helper.get_sample_description(s).split('Homo sapiens ')[-1])
            # trait_pval_df['trait'] = trait
            trait_pval_df['sig'] = trait_pval_df['pval'] < 0.05
            trait_pval_dfs.append(trait_pval_df)
            trait_pval_df.to_csv(os.path.join(out_dir, f'{trait}_sample_pvals.csv'), index=False)
            plot_pval_ranks(sample_pval_series=trait_pvals, trait_name=trait, sig_threshold=0.05, ax=ax,
                            meta_helper=meta_helper, cap_pvals_at_1=True)
            fig.savefig(os.path.join(out_dir, f'{trait}_rank_plots.pdf'), format='pdf')
        trait_pval_df_full = pd.concat(trait_pval_dfs, ignore_index=True)
        trait_pval_df_full.to_csv(os.path.join(out_dir, 'selected_trait_sample_pvals.csv'), index=False)
