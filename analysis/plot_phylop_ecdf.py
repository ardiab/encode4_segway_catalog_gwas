...
def main(out_dir):
    ...
    ecdf_plot_json_data_fpath = os.path.join(out_dir, 'ENCSR067NPP_label_phylop_ecdf_plot_data.json')
    ...
    for idx, (label, caas) in enumerate(label_caas_vals):
        ...
        label_color = const.LABEL_COLOR_MAP[interp_term] if interp_term != 'Quiescent' else (0, 0, 0)
    ...
    barplot_fig.savefig(os.path.join(out_dir, 'ENCSR067NPP_caas_hist.pdf'), format='pdf')
    barplot_fig.savefig(os.path.join(out_dir, 'ENCSR067NPP_caas_hist.png'))
    ...
    for label, label_dict in ecdf_data_dict.items():
        interp_term = label.split('_')[-1]
        label_color = const.LABEL_COLOR_MAP[interp_term] if interp_term != 'Quiescent' else (0.0, 0.0, 0.0)
        x_vals = label_dict['bins']
        y_vals = [0] + label_dict['counts']
        # Change cumulative densities that are e.g. 0.9999 to 1 (they're not exactly 1 because the code that made these
        # dicts threw out stuff higher than the 99th percentile, since it was significantly higher than everything else.
        y_vals[-1] = 1
    ...
    for interp_term, interp_color in const.LABEL_COLOR_MAP.items():
        sample_labels = list(sorted([k for k in ecdf_data_dict.keys() if k.split('_')[-1] == interp_term],
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'ENCSR067NPP_phylop_ecdf.pdf'), format='pdf')
    fig.savefig(os.path.join(out_dir, 'ENCSR067NPP_phylop_ecdf.png'), format='png')
