...
def create_barplot(metric_df, label_col, metric_col, sig_col, ylabel):
    ...
    for interp_term_center_x, interp_term in zip(interp_term_x_locs, ordered_interp_terms):
        ...
        bar_ticks = np.arange(interp_term_center_x - (total_width / 2) + (bin_width / 2),
        ...
        for snp_type_bar_x, snp_type, snp_hatch in zip(bar_ticks, snp_types, hatches):
            snp_freq_df = metric_df[(metric_df[label_col] == interp_term) & (metric_df[sig_col] == snp_type)]
            snp_freq = snp_freq_df[metric_col].values[0]
            assert snp_freq_df.shape[0] == 1
            ax.bar(snp_type_bar_x, snp_freq, color=label_color, edgecolor='black', width=bin_width, hatch=snp_hatch)
        ...
        for snp_type, snp_hatch in zip(snp_types, hatches):
            legend_patches.append(
                matplotlib.patches.Patch(facecolor=legend_color, label=legend_descs[snp_type],
                                         hatch=snp_hatch * 3 if snp_hatch is not None else None, edgecolor='black',
                                         linewidth=1.5)
            )
...
def main(out_dir):
    ...
    # snp_mean_caas_df = data_loader.load_snp_metric_df(metric_name='mean_caas', snp_region_side_length=10_000,
    #                                                   fake=False)
    ...
    for sample_id in tqdm.tqdm(pval_df.columns):
        sample_label_distrs = data_loader.load_sample_snp_label_distribution(sample_id=sample_id,
                                                                             snp_region_side_length=window_size,
                                                                             fake=False)
        sample_fake_label_distrs = data_loader.load_sample_snp_label_distribution(sample_id=sample_id,
                                                                                  snp_region_side_length=window_size,
                                                                                  fake=True)
        sample_label_genome_coverage = data_loader.load_sample_label_coverage(sample_id=sample_id, normalized=True)
        ...
        sample_sig_snps_proc = list(set(sample_sig_snps).intersection(set(sample_label_distrs.index.values)))
        sample_insig_snps_proc = list(set(sample_insig_snps).intersection(set(sample_label_distrs.index.values)))
        assert (len(sample_sig_snps) - len(sample_sig_snps_proc)) < 10
        assert (len(sample_insig_snps) - len(sample_insig_snps_proc)) < 10
        sample_sig_snps_proc_fake = list(set([snp + '_fake' for snp in sample_sig_snps]).intersection(
            set(sample_fake_label_distrs.index.values)))
        sample_insig_snps_proc_fake = list(set([snp + '_fake' for snp in sample_insig_snps]).intersection(
            set(sample_fake_label_distrs.index.values)))
        ...
        sample_label_mapping = {interp_term: [] for interp_term in list(const.LABEL_COLOR_MAP.keys())}
        ...
        for interp_term in list(const.LABEL_COLOR_MAP.keys()):
            ...
            interp_term_genome_coverage = sum([sample_label_genome_coverage[label] for label in interp_term_labels])
            for coverage_df, sig_desc, real in zip(
                    [sig_real_snp_coverages, insig_real_snp_coverages, sig_fake_snp_coverages, insig_fake_snp_coverages,
                ...
                interp_term_snp_coverages = coverage_df[interp_term_labels].sum(axis=1) / coverage_df.sum(axis=1)
                interp_term_enrichments = interp_term_snp_coverages / interp_term_genome_coverage
                proc_sig_desc = sig_desc + '_' + real
                enrichment_data_mean.append({'label': interp_term, 'enrichment': interp_term_enrichments.mean(),
                                             'sig': proc_sig_desc})

    df_enrichment_mean = pd.DataFrame(enrichment_data_mean).groupby(['label', 'sig'])['enrichment'].mean().reset_index()
    barplot_enrichment_df = df_enrichment_mean.groupby(['label', 'sig'])['enrichment'].mean().reset_index()
    ...
    barplot_enrichment_fig, barplot_enrichment_ax = create_barplot(
        metric_df=barplot_enrichment_df, label_col='label', metric_col='enrichment', sig_col='sig',
        ylabel='Enrichment within SNP Region\n$log_2 (observed/expected)$')
    barplot_enrichment_ax.axhline(0, color='black')

    barplot_enrichment_fig.savefig(os.path.join(out_dir, 'snp_label_enrichment_barplot_with_fake.pdf'), format='pdf')
    barplot_enrichment_fig.savefig(os.path.join(out_dir, 'snp_label_enrichment_barplot_with_fake.png'), format='png')
