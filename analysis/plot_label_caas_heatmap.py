...
def main(out_dir):
            ...
            if sample_n_interp_terms > 0:
                ...
                for idx, row in sample_df.sort_values(by=['caas'], ascending=False).iterrows():
                    for i in range(n_rows_per_integer_label):
                        heatmap_data.append({'sample': sample_id, 'interp_term': interp_term, 'label': row['label'],
                                             'caas': row['caas'], 'label_idx': label_idx, 'row': row_idx})
                        row_idx += 1
                    label_idx += 1
            else:
                ...
                n_rows_per_integer_label = int(n_rows_per_integer_label)
                for i in range(n_rows_per_integer_label):
                    heatmap_data.append({'sample': sample_id, 'interp_term': interp_term, 'label': 'N/A',
                                         'caas': np.nan, 'label_idx': 0, 'row': i})

        # for sample_id in meta_helper.iter_samples():
        #     sample_interp_term_sub = interp_term_sub[interp_term_sub['sample'] == sample_id].sort_values(
        #         by=['caas'], ascending=False)
        #     for i in range(max_count):
        #         try:
        #             sample_interp_caas = sample_interp_term_sub.iloc[i]['caas']
        #         except IndexError:
        #             sample_interp_caas = np.nan
        #         heatmap_data.append({'interp_term': interp_term, 'sample': sample_id, 'index': i,
        #                              'caas': sample_interp_caas})

    heatmap_df = pd.DataFrame(heatmap_data)
    heatmap_pivot = heatmap_df.pivot(index=['interp_term', 'row'], columns=['sample'], values=['caas'])
    ...
    clustermap = sns.clustermap(heatmap_pivot.values, dendrogram_ratio=[.05, .05], cmap='magma')
    ...
    sns.heatmap(heatmap_pivot.iloc[:, col_order], ax=ax, vmin=min_caas, vmax=max_caas, cmap=cmap, rasterized=True,
                cbar_kws={"shrink": 0.5, 'use_gridspec': False, 'location': 'top'})
    # ax.collections[0].colorbar.set_label('Label CAAS', fontsize=20)
    ...
    # ax.set_xlabel('Biosamples', fontsize=25, weight='bold')
    ax.set_xlabel('')
    ax.set_ylabel('')
    # fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'label_caas_heatmap_clustered.png'))
    fig.savefig(os.path.join(out_dir, 'label_caas_heatmap_clustered.pdf'), format='pdf')
