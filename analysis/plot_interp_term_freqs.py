...
def main(out_dir):
    ...
    for sample_id in meta_helper.iter_samples(show_progress=True):
        ...
        sample_label_coverage = data_loader.load_sample_label_coverage(sample_id=sample_id, normalized=True)
        for label, caas in sample_label_caas.items():
            label_caas_data.append({'sample': sample_id, 'label': label, 'interp_term': label.split('_')[-1],
                                    'caas': caas, 'coverage': sample_label_coverage[label]})
    ...
    interp_term_overall_counts = label_caas_df.groupby(['sample'])['interp_term'].value_counts().value_counts()
    ax.bar(interp_term_overall_counts.index, interp_term_overall_counts.values, linewidth=1.5, edgecolor='black')
    ax.tick_params(axis='both', which='major', labelsize=18)
    ax.set_xlabel('# occurrences of interpretation terms\n(for every interpretation term, sample pair)', fontsize=16,
                  weight='bold')
    ...
    fig.savefig(os.path.join(out_dir, 'snp_interp_term_counts_per_sample_overall.pdf'), format='pdf')
    fig.savefig(os.path.join(out_dir, 'snp_interp_term_counts_per_sample_overall.png'))
    ...
    interp_coverages = label_caas_df.groupby(['sample', 'interp_term'])['coverage'].sum().reset_index()
    i = 0
    for interp_term, color in const.LABEL_COLOR_MAP.items():
        ...
        # ax.hist(count_sub['label'].values, facecolor=color, label=interp_term, linewidth=1.5, edgecolor='black')
        ax.bar(count_val_counts.index.values, count_val_counts.values, facecolor=color, label=interp_term,
        ...
        ax_twin = ax.twinx()
        ax_twin.plot(count_coverages.index.values, count_coverages.values, 'X', color='black', markersize=16)
    ...
    fig.tight_layout()
    # fig.suptitle('Number of interpretation term occurrences per sample')
    fig.savefig(os.path.join(out_dir, 'snp_interp_term_counts_per_sample_per_term.pdf'), format='pdf')
    fig.savefig(os.path.join(out_dir, 'snp_interp_term_counts_per_sample_per_term.png'))
