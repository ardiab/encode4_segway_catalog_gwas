...
class PathHelper:
    ...
    def get_label_caas_json_fpath(self, sample_id):
        return os.path.join(self.get_sample_label_info_dir_path(sample_id), f'{sample_id}_label_caas.json')

    def get_label_phylop_percentiles_json_fpath(self, sample_id):
        return os.path.join(self.get_sample_label_info_dir_path(sample_id),
    ...
    def get_label_coverage_json_fpath(self, sample_id, normalized=True):
        if normalized:
            return os.path.join(self.get_sample_label_info_dir_path(sample_id), f'{sample_id}_label_coverage.json')
        else:
            return os.path.join(self.get_sample_label_info_dir_path(sample_id), f'{sample_id}_label_counts.json')

    def get_snp_bed_fpath(self, window_size, fake=False):
        assert fake in [True, False]
        return os.path.join(self.processed_snp_dir,
    ...
    def get_trait_snp_linkage_df_fpath(self, window_size):
        return os.path.join(self.processed_snp_dir, f'trait_snp_linkage_{window_size}_bidirectional_window.csv')
    ...
    def get_label_distr_df_fpath(self, sample_id, window_size, fake=False):
        assert fake in [True, False]
        return os.path.join(self.get_sample_intersection_dir(sample_id),
    ...
    def get_sample_test_encode_submission_sheet_dir(self, sample_id):
        return os.path.join(self.get_sample_base_encode_submission_sheet_dir(sample_id), 'test_submission')

    def get_sample_prod_encode_submission_sheet_dir(self, sample_id):
        return os.path.join(self.get_sample_base_encode_submission_sheet_dir(sample_id), 'production_submission')

    def get_sample_submission_pdf_figure_fpath(self, sample_id):
        return os.path.join(self.get_sample_base_encode_submission_sheet_dir(sample_id),
    ...
    def get_test_stat_df_fpath(self, metric_name, test_type, window_size, fake):
        assert fake in [True, False]
        fname = f'test_stats_{metric_name}_{test_type}_{window_size}_{"fake" if fake else "real"}.pkl'
        return os.path.join(self.enrichment_dir, fname)

    def get_effect_size_df_fpath(self, metric_name, test_type, window_size, fake):
        assert fake in [True, False]
        fname = f'effect_size_{metric_name}_{test_type}_{window_size}_{"fake" if fake else "real"}.pkl'
        return os.path.join(self.enrichment_dir, fname)
