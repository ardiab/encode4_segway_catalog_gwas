...
class MetaHelper:
    ...
    def validate_meta_df(cls, meta_df):
        assert meta_df.notnull().all().all(), f'The metadata DF contains null values.'
        ...
        for fp in meta_df[cls.BEDPATH_COL].values:
            ...
            assert fp.endswith('.bed') or fp.endswith('.bed.gz'), \
                f'All bed files must have an extension of ".bed" or ".bed.gz".'
        if meta_df[cls.SAMPLE_DESCRIPTION_COL].duplicated().any():
            warnings.warn(f'Found duplicated sample descriptions.')
    ...
    def iter_samples(self, show_progress=False):
        iterable = tqdm(self.meta_df[self.SAMPLE_ID_COL].values) if show_progress else \
            self.meta_df[self.SAMPLE_ID_COL].values
    ...
    def get_sample_description(self, sample_id):
        self.validate_sample_id(sample_id)
        return self.meta_df.set_index(self.SAMPLE_ID_COL).loc[sample_id, self.SAMPLE_DESCRIPTION_COL]
