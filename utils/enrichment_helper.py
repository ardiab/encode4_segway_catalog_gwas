...
class EnrichmentHelper:
    ...
    def get_max_caas_series(cls, label_distr_df, label_caas_series):
        ...
        assert set(label_distr_df.columns) == set(label_caas_series.index)
        # Check that all columns are numeric data types.
        assert all([pd.api.types.is_numeric_dtype(label_distr_df[c]) for c in label_distr_df.columns])
        return label_distr_df.astype(bool).astype(int).apply(lambda row: (row * label_caas_series).max(), axis=1)
    ...
    def get_signedrank_pvals_per_trait_across_biosamples(cls, metric_df, print_progress=False):
        ...
        if print_progress:
            return rank_df.groupby('trait').progress_apply(lambda trait_df: trait_df.drop(columns=['trait']).apply(
                lambda col: wilcoxon(col.values - median_rank, alternative='greater')[1]))
        else:
            return rank_df.groupby('trait').apply(lambda trait_df: trait_df.drop(columns=['trait']).apply(
                lambda col: wilcoxon(col.values - median_rank, alternative='greater')[1]))
    ...
    def get_ranksum_pvals_per_trait_across_biosamples(cls, metric_df, print_progress=False):
        ...
        grouper = tqdm(metric_df.reset_index().groupby('trait')) if print_progress else metric_df.reset_index().groupby('trait')
        for trait, trait_df in grouper:
            for biosample_name in biosample_names:
                ...
                _, pval = ranksums(trait_df.loc[:, biosample_name], trait_df.loc[:, other_biosamples], alternative='greater')
    ...
    def get_ranksum_pvals_per_biosample_across_traits(cls, metric_df, print_progress=False):
        ...
        for biosample_name in cols:
            ...
            for trait in tqdm(trait_names):
                ...
                other_ranks = biosample_series.loc[list(trait_names.difference(trait))].dropna().values
